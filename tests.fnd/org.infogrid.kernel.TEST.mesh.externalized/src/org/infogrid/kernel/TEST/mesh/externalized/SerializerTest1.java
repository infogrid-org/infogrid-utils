            ...
            for( int j=0 ; j<testData[i].length ; ++j ) {
                ...
                for( int k=0 ; k<testData[i][j].theValues.length ; ++k ) {
                    ...
                    try {
                        ...
                        ++errorCount;
                        reportError( "ERROR: element " + j + " of type " + original.getClass() + " threw exception of "
                                     + ex.getClass() + " during "
                                     + ( (encoded == null) ? "encoding" : "decoding" ),
                                     ex );
    ...
    protected static TestCase [] blobValueTestData = new TestCase[] {
            new TestCase(
                    ...
                    new BlobValue[] {
                         ...
                         BlobValue.create( testString ),
                         BlobValue.create( testString, "text/html" ),
                         BlobValue.create( testString + testString + testString + testString ),
                         BlobValue.create( testBytesShort, "something/unknown" ),
                         BlobValue.create( testBytesLong, "something/unknown" ),
                         BlobValue.create( new byte[] { (byte) 1 }, "a/b" ),
