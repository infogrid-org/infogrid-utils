 ...
 */
public class LidLocalPersonaWithCredentialsMapper
        extends
            DefaultHandler
        implements
            StoreEntryMapper<String,LidLocalPersonaWithCredentials>,
            LidLocalPersonaWithCredentialsTags
{
    private static final Log log = Log.getLogInstance( LidLocalPersonaWithCredentialsMapper.class ); // our own, private logger
     ...
     */
    public LidLocalPersonaWithCredentialsMapper(
     ...
     */
    public LidLocalPersonaWithCredentials decodeValue(
            String     key,
            StoreValue value )
        throws
            ...
            SAXException
    {
        ...
        if( PERSONA_TAG.equals( qName )) {
            ...
            theObjectBeingParsed = LidLocalPersonaWithCredentials.create(
            ...
            SAXException
    {
        ...
        if( PERSONA_TAG.equals( qName )) {
            ...
            theObjectBeingParsed.getPersona().setAttribute(
                    theCurrentAttribute,
     ...
     * @throws SAXParseException thrown if a prob
     */
    ...
    public long getTimeCreated(
            LidLocalPersonaWithCredentials value )
    {
        return -1L;
    }
     ...
     */
    public byte [] asBytes(
            LidLocalPersonaWithCredentials persona )
        throws
            StoreValueEncodingException
    {
        StringBuilder buf = new StringBuilder();
        buf.append( "<" ).append( PERSONA_TAG );
        buf.append(  " " ).append( IDENTIFIER_TAG ).append( "=\"" ).append( XmlUtils.escape( persona.getPersona().getIdentifier() )).append( "\">\n");

        for( String name : persona.getPersona().getAttributes().keySet() ) {
            String value = persona.getPersona().getAttribute( name );
        ...
        for( LidCredentialType type : persona.getCredentials().keySet() ) {
            String value = persona.getCredentialFor( type );
            
            buf.append( "<" ).append( CREDENTIAL_TAG );
            buf.append(  " " ).append( IDENTIFIER_TAG ).append( "=\"" ).append( XmlUtils.escape( type.getName() )).append( "\">\n");
            buf.append( XmlUtils.escape( value ));
     ...
     * The encoding to use.
     */
    public static final String ENCODING = LidLocalPersonaWithCredentialsMapper.class.getName();
    ...
    /**
     * The LidLocalPersonaWithCredentials instance currently being parsed.
     */
    protected LidLocalPersonaWithCredentials theObjectBeingParsed;
