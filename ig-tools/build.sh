...
run_command() {
	if [ "${verbose}" = 0 ]; then
		...
		$* >${TMPFILE} 2>&1
			...
			grep -i warning ${TMPFILE} | egrep -v 'unchecked call to add\(E\) as a member of the raw type java\.util\.(Vector|List)' | egrep -v '[0-9]+[ \t]+warnings$' | grep -v 'is internal proprietary API and may be removed in a future release'
