            ...
            MeshObjectSetProbeTags
{
     ...
     *         Probe developer incorrectly assigned duplicate Identifiers to created MeshObjects
            ...
            URISyntaxException
    {
        ...
        theMeshBase  = mb;
        theModelBase = mb.getModelBase();

        try {
            theBufferedObjects = new ArrayList<ExternalizedMeshObject>();
            
            // first parse
            theParser.parse( theInputStream, this );
            ...
            for( ExternalizedMeshObject currentObject : theBufferedObjects ) {
                MeshObject realCurrentObject = life.createMeshObject(
                        mb.getMeshObjectIdentifierFactory().fromExternalForm( currentObject.getIdentifier() ),
                        lookupEntityTypes( currentObject.getMeshTypes(), mb.getModelBase() ),
                ...
                for( int i=currentObject.thePropertyTypes.size()-1 ; i>=0 ; --i ) {
                    PropertyType  type  = lookupPropertyType( currentObject.thePropertyTypes.get( i ), mb.getModelBase() );
            ...
            for( ExternalizedMeshObject currentObject : theBufferedObjects ) {
                MeshObject realCurrentObject = mb.findMeshObjectByIdentifier(
                        mb.getMeshObjectIdentifierFactory().fromExternalForm( currentObject.getIdentifier() ));

                for( ExternalizedMeshObject.ExternalizedRelationship currentRelationship : currentObject.theRelationships ) {
                    MeshObject otherSide = mb.findMeshObjectByIdentifier(
                            mb.getMeshObjectIdentifierFactory().fromExternalForm( currentRelationship.getIdentifier() ));
                    ...
                    try {
                        realCurrentObject.blessRelationship( lookupRoleTypes( currentRelationship.theRoleTypes, mb.getModelBase() ), otherSide );
